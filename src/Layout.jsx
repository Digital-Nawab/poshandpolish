...
import { Drawer, Button, Typography, IconButton, List, } from "@material-tailwind/react";
...
const Layout = ({ children }) => {
  ...
  const handleSubmitPopup = async (event) => {
    ...
    try {
      ...
      const response = await axios.post("https://admin.theposhandpolished.com/api/book-appointment", dataToSubmit);
  
      if (response.data.status) {
         toast.success(`Hi ${formDataPopup.name}, your appointment enquiry has been submitted successfully.`);
  ...
  return (
    ...
    {/* <div className="grid place-items-center h-dvh bg-zinc-900/15">
        ...
        <div className='container mx-auto'>
          <header className="flex justify-between items-center ">
            ...
            <nav className="hidden md:flex gap-10 text-sm">
              <Link to="/" className="hover:text-[#ffcc6e] text-lg hover:underline hover:decoration-yellow-400 h2">Home</Link>
              <Link to="/about" className="hover:text-[#ffcc6e] text-lg  hover:underline hover:decoration-yellow-400 h2">About</Link>
              <Menu allowHover={true}>
                <MenuHandler>
                <h2 to="/services" className="hover:text-[#ffcc6e] text-lg cursor-pointer  hover:underline hover:decoration-yellow-400 h2">Services</h2>

                </MenuHandler>
                <MenuList>
                  {servicesMenu && servicesMenu.map((service) => (
                    <MenuItem key={service._id}>
                      <Link to={`/service/${service.slug_url}`}>{service.title}</Link>
                    </MenuItem>
                  ))}
                </MenuList>
              </Menu>
              <Link to="/gallery" className="hover:text-[#ffcc6e]  text-lg  hover:underline hover:decoration-yellow-400 h2">Gallery</Link>
              <Link to="/contact" className="hover:text-[#ffcc6e] text-lg hover:underline hover:decoration-yellow-400 h2">Contact</Link>
              <Link to="/offer" className="text-lg tracking-widest bg-yellow-600 font-semibold rounded-xl px-2 !text-[#63682F] hover:text-yellow-400 h2 hover:underline hover:decoration-yellow-400 
      animate-blink transition-all duration-300 ease-in-out" > Offers</Link>
            </nav>
            <div className="md:hidden block z-50 bg-[#63682F]">
              <React.Fragment>
                <Button className='bg-[#63682F] shadow-none hover:shadow-none' onClick={openDrawer}><i class="fa fa-2x fa-bars" aria-hidden="true"></i></Button>

                <Drawer className='bg-[#63682F]' open={open} onClose={closeDrawer}>
                  ...
                  <List>
                    ...
                    <Link to="/" className="hover:text-[#ffcc6e] text-lg w-full  hover:underline hover:decoration-yellow-400 h2">Home</Link>

                    <Link to="/about" className="hover:text-[#ffcc6e] w-full  text-lg  hover:underline hover:decoration-yellow-400 h2">About</Link>

                    <Accordion className='p-0 border-none' open={open2 === 1} icon={<Icon id={1} open={open2} />}>
                      <AccordionHeader className='p-0 border-none' onClick={() => handleOpen(1)}>  <h2 to="/about" className="hover:text-[#ffcc6e] w-full  text-lg  hover:underline hover:decoration-yellow-400 h2">About</h2>
                      </AccordionHeader>
                    ...
                    <Link to="/gallery" className="hover:text-[#ffcc6e] w-full text-lg  hover:underline hover:decoration-yellow-400 h2">Gallery</Link>

                    <Link to="/contact" className="hover:text-[#ffcc6e] w-full text-lg hover:underline hover:decoration-yellow-400 h2">Contact</Link>


                  </List>
                  <Button className="mt-3 ml-5 bg-[#ffcc6e] text-[#63682F]" size="sm">
                    Book Appointment
                  </Button>
                </Drawer>
              </React.Fragment>
      ...
      {isPopupOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex z-50 items-center justify-center">
          <div className="relative bg-white w-80 p-4 text-[#63682f] rounded-lg">
            ...
            <h2 className="text-lg font-bold text-[#63682f] mb-4">Enquiry Form</h2>
      ...
      <div className='reative'>
        <button className="fixed top-1/2 right-[-66px] animate-bgBlink transition-all duration-300 ease-in-out transform -translate-y-1/2 shadow-lg rotate-90 z-50 text-white px-4 py-2 bg-[#63682f] rounded hover:bg-blue-700" onClick={() => handlePopup({title : 'Book Appointment'})} >
          Book Appointment
        </button>


        <div className="fixed bottom-10 right-2 transform -translate-y-1/2 flex flex-col gap-2 z-50">
          <a href='tel:+919559165805'
          ...
          <a href='https://wa.me/919956465805' className="flex items-center justify-center bg-green-500 text-white rounded-full w-14 h-14 hover:bg-green-700">
        ...
        <div className="container mx-auto px-6 lg:px-20">
            ...
            <div>
              ...
              <div className="space-y-2">
                <p className="flex items-center text-white">
                  <span className="mr-2"><i className='fa fa-phone'></i></span> +91 9956465805, 9559165805
                </p>
                <p className="flex items-center text-white">
                  <span className="mr-2"><i className='fa fa-envelope'></i></span> poshnpolishedsalon@gmail.com
                </p>
                <p className="flex items-center text-white">
                  <span className="mr-2"><i className='fa fa-map'></i></span> B-40, Mandir Marg, opposite House of Johnson, Mahanagar
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 border-t border-gray-700 pt-4 flex flex-col md:flex-row justify-between">
            ...
            <div className="flex space-x-4 text-sm mt-4 md:mt-0">
              ...
              <a href="tel:+919559165805" className="hover:underline text-white">
                <i className="fa fa-phone"></i>
              </a>
              <a href="mailto:poshnpolishedsalon@gmail.com" className="hover:underline text-white">
                <i className="fa fa-envelope"></i>
              </a>
              <a href="https://wa.me/919956465805" className="hover:underline text-white">
