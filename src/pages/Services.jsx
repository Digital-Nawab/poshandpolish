...
function Services() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://admin.theposhandpolished.com/api/service/${url.slug_url}`);
  ...
  return (
    <>
      <Layout>
        {/* About Section */}
        {aboutData.map((item) => (
          <section key={item.id} data-aos="fade-down" className="bg-[#f7f3e8] text-white about-section py-16 px-4">
        ...
        <div class="bg-light-gradient bg-gray-100 w-full h-full py-16 px-4 ">
          <div className="container mx-auto px-4">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-gray-800">{serviceData.title}</h2>
            ...
            {offerData.map((item) => (
              <SwiperSlide key={item.id}>
                ...
                <img loading="lazy" src={`https://admin.theposhandpolished.com/${item.image}`} alt={item.title} />
                  <button className="w-full py-2 bg-[#63682f] text-white text-lg font-semibold transition-all duration-300 hover:bg-[#63682f] hover:shadow-xl" onClick={() => handlePopup({title : item.title})} >
                    Book Appointment
                  </button>
                </div>
              </SwiperSlide>
